        ...
        {template.secciones.map((section) => {
          ...
          const renderRow = (row: (typeof layout)[number]) => {
            ...
            if (fields.length === 1 && fields[0].tipo === 'encabezado') {
              ...
              return (
                <View key={row.id} wrap={false} style={{ marginBottom: 4 }}>
            ...
            return (
              <View key={row.id} wrap={hasExpandible ? undefined : false} style={{ flexDirection: 'row', marginBottom: 5, alignItems: 'flex-start' }}>
